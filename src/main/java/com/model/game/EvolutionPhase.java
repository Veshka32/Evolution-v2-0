...
class EvolutionPhase {
    ...
    static void processMove(Game game, Move move) throws GameException {
        ...
        switch (move.getMove()) {
            ...
            case DELETE_PROPERTY:
                ...
                break;
            case PLAY_PROPERTY:
                if (Property.isPropertyDouble(move.getProperty()))
                    processDoubleProperty(game, move);
                else {
                    processSimpleProperty(game, move);
                }
        }
        if (game.getPhase().equals(Phase.EVOLUTION))
            game.switchPlayerOnMove(); //if new phase, do not switch player, because playersTurn is update
    }
    ...
    private static void processDoubleProperty(Game game, Move move) throws GameException {
        ...
        if (id1 == 0 || id2 == 0)
            throw new GameException("You must pick two animals to play property " + move.getProperty());
